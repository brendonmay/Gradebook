...
if (Meteor.isServer) {
    Meteor.methods({
        ...
        'calculatedgrades.modifiedCategoryAmount'(ownerId, courseId, assessmentId, modifiedCategory, categoryTotal, oldCategoryTotal) {
            var studentsArray = CalculatedGrades.findOne({ ownerId, courseId }).students;
            var assessmentTypeId = assessmentId;
            var finalEvaluation = true;
            //check type of evaluation
            if (assessmentId.indexOf("-") > -1) {
                assessmentTypeId = assessmentId.slice(0, assessmentId.indexOf("-"));
                finalEvaluation = false;
            }
            var categoryKey = modifiedCategory + "Grade";
            var newGrade;
            var studentScore;
            for (var i = 0; i < studentsArray.length; i++) {
                ...
                var currentGrades = studentsArray[i].currentGrades;
                for (var z = 0; z < currentGrades.length; z++) {
                    //if dealing with final evaluation
                    if (finalEvaluation) {
                        //console.log("You modified a final evaluation");
                        if (currentGrades[z].assessmentTypeId == assessmentTypeId) {
                            var studentsData = Students.findOne({ ownerId, courseId }).students;
                            for (var q = 0; q < studentsData.length; q++) {
                                if (studentsData[q].studentId == studentId) {
                                    var grades = studentsData[q].grades;
                                    for (var u = 0; u < grades.length; u++) {
                                        if (grades[u].assessmentId == assessmentId) {
                                            studentScore = grades[u][modifiedCategory];
                                            if (studentScore != "N/A") {
                                                updateGrades = true;
                                            }
                                            // console.log("studentScore is: " + studentScore);
                                            // console.log("will we update grade? " + updateGrades);
                                            u = grades.length;
                                        }
                                    }
                                    q = studentsData.length;
                                }
                            }
                            if (updateGrades) {
                                if (categoryTotal == "N/A"){
                                    Meteor.call('students.removeCategory', ownerId, courseId, modifiedCategory, assessmentTypeId);
                                }

                                //Determine newGrade
                                newGrade = Number(((studentScore / categoryTotal) * 100).toFixed(2));
                                //console.log("newGrade = "+ newGrade);

                                //update studentsArray
                                if (isNaN(newGrade) || newGrade == null) {
                                    delete studentsArray[i].currentGrades[z].assessmentTypeGrade[categoryKey];
                                }
                                else {
                                    studentsArray[i].currentGrades[z].assessmentTypeGrade[categoryKey] = newGrade;
                                }
                                //update categoryGrades only on modified category
                                newCategoryGrade = determineOverallCategoryGrade(ownerId, courseId, studentId, modifiedCategory, studentsArray, i);
                                //console.log("newCategoryGrade = " + newCategoryGrade);

                                if (isNaN(newCategoryGrade) || newCategoryGrade == Infinity || newCategoryGrade == null) {
                                    if (studentsArray[i].categoryGrades[categoryKey] != null) {
                                        delete studentsArray[i].categoryGrades[categoryKey];
                                    }
                                }
                                else {
                                    studentsArray[i].categoryGrades[categoryKey] = newCategoryGrade;
                                }
                            }
                            z = currentGrades.length;
                        }
                    }
                    //not dealing with final
                    else {
                        if (currentGrades[z].assessmentTypeId == assessmentTypeId) {
                            var assessments = currentGrades[z].assessments;
                            var newAssessmentTypeGrade;
                            ...
                            var totalAssessmentStudentScore = 0;
                            var updateGrades = false;

                            for (var w = 0; w < assessments.length; w++) {
                                if (assessments[w][categoryKey] != null) {
                                    totalAssessments++;
                                    if (assessments[w].assessmentId != assessmentId) {
                                        totalAssessmentStudentScore = totalAssessmentStudentScore + assessments[w][categoryKey];
                                    }
                                ...
                                if (assessments[w].assessmentId == assessmentId) {
                                    ...
                                    //update studentsArray
                                    if (isNaN(newGrade) || newGrade == null) {
                                        delete studentsArray[i].currentGrades[z].assessments[w][categoryKey];
                                    }
                                    else {

                                        studentsArray[i].currentGrades[z].assessments[w][categoryKey] = newGrade;
                                        totalAssessmentStudentScore = totalAssessmentStudentScore + newGrade;
                                    }
                                }
                            }
                            if (updateGrades) {
                                if (categoryTotal == "N/A"){
                                    Meteor.call('students.removeCategory', ownerId, courseId, modifiedCategory, assessmentTypeId, assessmentId);
                                }
                                
                                if (oldCategoryTotal == null || oldCategoryTotal == "N/A" || isNaN(oldCategoryTotal)) {
                                    totalAssessments++;
                                }
                                newAssessmentTypeGrade = totalAssessmentStudentScore / totalAssessments;
                                if (isNaN(newAssessmentTypeGrade) || newAssessmentTypeGrade == Infinity || newAssessmentTypeGrade == null) {
                                    if (studentsArray[i].currentGrades[z].assessmentTypeGrade[categoryKey] != null && totalAssessments < 1) {
                                        delete studentsArray[i].currentGrades[z].assessmentTypeGrade[categoryKey]; 
                                    }
                                }
                                else {
                                    studentsArray[i].currentGrades[z].assessmentTypeGrade[categoryKey] = newAssessmentTypeGrade;
                                }
                    ...
                    if (currentGrades[z].assessmentTypeId == assessmentTypeId) {
                        ...
                        for (var w = 0; w < assessments.length; w++) {
                            if (assessments[w].assessmentId == assessmentId) {
                                ...
                                if (assessments[w].KGrade || assessments[w].KGrade == 0) {
                                    K = true;
                                }
                                if (assessments[w].AGrade || assessments[w].AGrade == 0) {
                                    A = true;
                                }
                                if (assessments[w].TGrade || assessments[w].TGrade == 0) {
                                    T = true;
                                }
                                if (assessments[w].CGrade || assessments[w].CGrade == 0) {
                                    C = true;
                                }
                                //delete the assessment
                                studentsArray[i].currentGrades[z].assessments.splice(w, 1);

                                //check if that was the only assessment of that type
                                if (studentsArray[i].currentGrades[z].assessments.length == 0) {
