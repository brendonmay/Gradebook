...
if (Meteor.isServer) {
    Meteor.methods({
        ...
        'students.removeCategory'(ownerId, courseId, category, assessmentTypeId, assessmentId) {
            if(assessmentId){
            ...
            for (i = 0; i < newStudentsArray.length; i++) {
                var grades = newStudentsArray[i].grades;
                for (z = 0; z < grades.length; z++) {
                    if (grades[z].assessmentId == assessmentTypeId) {
                        newStudentsArray[i].grades[z][category] = "N/A";
                        z = grades.length;
                    }
                }
            }
