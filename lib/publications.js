...
if (Meteor.isServer) {
    Meteor.methods({
                ...
                if (result.success) {
                    ...
                    newSubscription.then(function (result) {
                        if (result && result.success == true) {
                            var todayDate = CurrentDate.findOne({}).date;
                            Meteor.users.update(
                                { _id: userID },
                                {
                                    $set:
        ...
        'refreshPaymentInformation'() {
            var newSubscriptionObj = Meteor.users.findOne({ _id: Meteor.userId() }).subscribed;
            newSubscriptionObj.transactionFailed = "";
...
function startUp() {
    ...
    var password = encodeURIComponent("vedzxtslalbuqknb"); //pw without 2-step verification is J.Cole2477
